...
class MonotonicFunc(BSplineBase):
    ...
    def __init__(self, out_features=1, num_grids=10, k=3, grid_range=(0, 1), device='cpu'):
        ...
        self.C_raw = nn.Parameter(torch.full((self.out_features, self.in_features), 1e-3, device=device))
        self.alpha_raw = nn.Parameter(torch.full((self.out_features, self.in_features), 1e-3, device=device))
    ...
    def forward(self, x):
        ...
        # 3. Exponential Base Term: (A/alpha) * (1 - exp(-alpha*s))
        C = F.softplus(self.C_raw)  # (out_features, 1) - Extra initial slope
        # alpha must be strictly positive to avoid division by zero
        alpha = F.softplus(self.alpha_raw) + 1e-6  # (out_features, 1) - Decay rate, adding epsilon
        # A_over_alpha shape (out_features, 1)
        A_over_alpha = C / alpha
        A_over_alpha_bcast = A_over_alpha.squeeze(-1).unsqueeze(0) # (1, out_features)
        
        # alpha_bcast shape (1, out_features)
        alpha_bcast = alpha.squeeze(-1).unsqueeze(0) 
        
        # exp(-alpha*s) calculation: x (batch, 1) * alpha_bcast (1, out_features)
        # Result shape (batch, out_features)
        exp_term_val = torch.exp(-alpha_bcast * x)
        
        # Exponential base term calculation
        exp_base_term = A_over_alpha_bcast * (1 - exp_term_val) # (batch, out_features)
        ...
        # Total
        f = linear_term + spline_term + exp_base_term  # (batch, out_features)
        return f
...
class MonotonicKANLayer(nn.Module):
    ...
    def __init__(self, hidden_dim=10, num_grids=10, k=3, t_range=(0, 1), device='cpu'):
        ...
        self.a_s_func = MonotonicFunc(out_features=1, num_grids=num_grids, k=k, device=device)
        # H_k(s) functions: monotonic with output size hidden_dim
        self.H_s_func = MonotonicFunc(out_features=hidden_dim, num_grids=num_grids, k=k, device=device)
...
if __name__ == "__main__":
    ...
    model = MonotonicKANLayer(hidden_dim=10, num_grids=20, k=3, t_range=(0,5), device=device)
    ...
    print(f"d range: [{d.min().item()}, {d.max().item()}]")
    print(f"w_raw range: [{w_raw.min().item()}, {w_raw.max().item()}]")
